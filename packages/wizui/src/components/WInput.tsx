...
export interface WInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, "size"> {
  color?: Color
  variant?: Variant
  size?: Size
  icon?: IconProp
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className={cx(
          "min-w-0 flex-1 appearance-none border-0 bg-transparent p-0 outline-none",
          "placeholder:text-neutral-400 disabled:cursor-not-allowed",
          type === "file" && "file:mr-2 file:border-0 file:bg-transparent file:text-sm file:font-medium",
          ui?.base,
        )}
        {...props}
      />
      {trailingNode != null ? <span className={cx("inline-flex shrink-0 items-center", ui?.trailing)}>{trailingNode}</span> : null}
    </div>
  )
}
